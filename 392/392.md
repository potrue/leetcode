...
この問題のように文字の種類が固定されている（26種類だけ）の場合は、tにおけるすべてのindexにおいて次にある文字が出てくるindexが書いてある配列を最初に構成してしまうことで、計算量がo(|s|)にすることもできる。（ただしその配列の構成にO(|t| × 文字の種類)の時間とメモリがかかるので大体のユースケースで見合ってなさそう）
